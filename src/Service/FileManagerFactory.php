...
class FileManagerFactory implements FactoryInterface
{
    public function __invoke(ContainerInterface $serviceLocator, $requestedName, array $options = null)
    {
        ...
        if (!isset($thumbnailTypes['large'])
            ...
            throw new ConfigException('Missing the large, medium, or square thumbnail type configuration');
        }
        foreach ($thumbnailTypes as $thumbnailType) {
            if (!isset($thumbnailType['constraint'])) {
                throw new ConfigException('Missing constraint for a thumbnail type configuration');
            }
        }
        if (!isset($fileManager['thumbnail_fallbacks'])
            || !is_array($fileManager['thumbnail_fallbacks'])
        ) {
            throw new ConfigException('Missing thumbnail fallbacks configuration');
        }
        ...
        if (!isset($thumbnailFallbacks['default'])
            ...
            throw new ConfigException('Missing default thumbnail fallback configuration');
        }
        if (!isset($thumbnailFallbacks['fallbacks'])
            || !is_array($thumbnailFallbacks['fallbacks'])
        ) {
            throw new ConfigException('Missing thumbnail fallback configuration');
        }
